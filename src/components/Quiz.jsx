    ...
    <div className="min-h-screen bg-cream font-retro">
      <div className="relative max-w-3xl mx-auto px-3 sm:px-4 py-6 sm:py-10">
        ...
        <div className="mb-5 sm:mb-6 px-1">
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-black leading-tight uppercase text-ink">
            Quiz
          </h1>
        ...
        {!finished ? (
          <div className="flex flex-col gap-4 sm:gap-5">
            ...
            <div className="rounded-retro overflow-hidden bg-white border-2 border-ink shadow-retro">
              {/* Card header */}
              <div className="px-4 sm:px-7 py-3 sm:py-5 border-b-2 border-ink flex flex-wrap items-center gap-2 bg-cream-dark">
                <span
                  className="inline-block text-[10px] sm:text-xs font-bold px-2.5 sm:px-3 py-1 rounded-retro border-2 border-ink uppercase tracking-wide text-ink"
                  style={{ background: bloomColors[question.bloomLevel] }}
                >
                  {question.bloomLevel}
                </span>
                <span className="inline-block text-[10px] sm:text-xs font-bold px-2.5 sm:px-3 py-1 rounded-retro border-2 border-ink uppercase tracking-wide text-ink bg-teal">
                  Q {activeIndex + 1} / {questions.length}
                </span>
                <div className="ml-auto shrink-0 flex items-center gap-1.5 text-[10px] sm:text-xs font-bold px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-retro border-2 border-ink bg-white text-ink">
                  Score: {score}
                </div>
              </div>
              ...
              <div className="px-4 sm:px-7 py-5 sm:py-6">
                <h2 className="text-base sm:text-xl font-black mb-4 sm:mb-6 text-ink leading-snug">
                  {question.question}
                </h2>
                  ...
                  {question.options.map((option, i) => {
                    ...
                    return (
                      <button
                        ...
                        disabled={answered}
                        className="relative flex items-center gap-2.5 sm:gap-3 px-3 sm:px-4 py-2.5 sm:py-3 rounded-retro text-left border-2 transition-all duration-100 w-full"
                        style={{
                          background: bg,
                          borderColor: border,
                      ...
                      >
                        <div
                          className="w-6 h-6 sm:w-7 sm:h-7 rounded-full flex items-center justify-center shrink-0 text-[10px] sm:text-xs font-black border-2 border-ink"
                          style={{
                        ...
                        <p className="text-xs sm:text-sm font-semibold text-ink flex-1">
                          {option}
                        </p>
                      </button>
                    );
                ...
                {answered && question.explanation && (
                  <div className="mt-4 sm:mt-5 px-3 sm:px-4 py-2.5 sm:py-3 rounded-retro border-2 border-dashed border-ink bg-cream">
                    <p className="text-[10px] sm:text-xs font-bold tracking-widest uppercase mb-1 text-ink/60">
                      Explanation
                    </p>
            ...
            {/* Navigation — dots scroll horizontally on mobile instead of squeezing */}
<div className="flex flex-col sm:flex-row items-center justify-between gap-3">
  <button
    onClick={handlePrevious}
    disabled={isFirst}
    className="order-1 flex items-center gap-2 px-4 sm:px-5 py-2.5 sm:py-3 rounded-retro text-xs sm:text-sm font-bold border-2 border-ink transition-all w-full sm:w-auto"
    style={{
  ...
  <div
    className="order-3 sm:order-2 flex items-center gap-2 overflow-x-auto max-w-full py-1 px-1 sm:flex-1 sm:justify-center"
  ...
  <button
    onClick={handleNext}
    disabled={!answered}
    className="order-2 sm:order-3 flex items-center gap-2 px-4 sm:px-5 py-2.5 sm:py-3 rounded-retro text-xs sm:text-sm font-black border-2 border-ink transition-all hover:translate-x-[1px] hover:translate-y-[1px] active:translate-x-[3px] active:translate-y-[3px] w-full sm:w-auto"
    style={{
  ...
  >
    {isLast ? "Finish" : "Next →"}
          ...
          /* Results screen */
          <div className="rounded-retro overflow-hidden bg-white border-2 border-ink shadow-retro">
            <div className="px-4 sm:px-7 py-4 sm:py-5 border-b-2 border-ink bg-cream-dark">
              <span className="inline-block text-[10px] sm:text-xs font-bold px-2.5 sm:px-3 py-1 rounded-retro border-2 border-ink uppercase tracking-wide text-ink bg-coral">
                Results
              </span>
              <h2 className="text-xl sm:text-2xl font-black mt-2 uppercase text-ink">
                Quiz Complete
              </h2>
            </div>

            <div className="px-4 sm:px-7 py-6 sm:py-8 flex flex-col items-center gap-2 border-b-2 border-ink">
              <p className="text-4xl sm:text-5xl font-black text-ink">
            ...
            {/* Answer review */}
            <div className="px-4 sm:px-7 py-5 sm:py-6 flex flex-col gap-3 sm:gap-4">
              <p className="text-[10px] sm:text-xs font-bold tracking-widest uppercase text-ink/60">
                Review Your Answers
              </p>
              ...
              {questions.map((q, i) => {
                ...
                return (
                  <div
                    key={q.id}
                    className="rounded-retro border-2 border-ink p-3 sm:p-4"
                    style={{
                  ...
                  >
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 sm:gap-3 mb-3">
                      <p className="text-xs sm:text-sm font-black text-ink flex-1">
                        {i + 1}. {q.question}
                      </p>
                      <span
                        className="self-start shrink-0 text-[10px] sm:text-xs font-bold px-2.5 py-1 rounded-retro border-2 border-ink uppercase"
                        style={{
                    ...
                    <div className="flex flex-col gap-1.5">
                      {q.options.map((option, j) => {
                        ...
                        return (
                          <div
                            key={j}
                            className="flex flex-wrap items-center gap-2 sm:gap-2.5 px-2.5 sm:px-3 py-2 rounded-retro border-2 border-ink text-[11px] sm:text-xs font-semibold text-ink"
                            style={{ background: bg }}
                          >
                            ...
                            {isUserPick && (
                              <span className="text-[9px] sm:text-[10px] font-bold uppercase text-ink/60">
                                Your answer
                              </span>
                            )}
                          </div>
                    ...
                    {q.explanation && (
                      <p className="text-[11px] sm:text-xs leading-relaxed mt-3 text-ink/70">
                        {q.explanation}
                      </p>
                    )}
                  </div>
            ...
            <div className="px-4 sm:px-7 pb-5 sm:pb-7 pt-2 border-t-2 border-ink flex items-center justify-between gap-3">
              <button
                onClick={handleRestart}
                className="flex items-center gap-2 px-4 sm:px-5 py-2.5 sm:py-3 rounded-retro text-xs sm:text-sm font-bold border-2 border-ink transition-all hover:translate-x-[1px] hover:translate-y-[1px] active:translate-x-[3px] active:translate-y-[3px]"
                style={{
